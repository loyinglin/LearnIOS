...
@property (nonatomic, strong) dispatch_queue_t mGLQueue;
@end
...
- (void)viewDidLoad {
    ...
    _mGLQueue = dispatch_queue_create("loying.lin", NULL);
...
- (void)renderNew {
    ...
    dispatch_source_set_event_handler(timer, ^{
        ...
        if (last != 0) {
            ...
            uint64_t elapsedNano = (end - last) * timebaseInfo.numer / timebaseInfo.denom;
            double elapsedSeconds = (double)elapsedNano / 1000000000.0;
            NSLog(@"当前timer回调速度%.4lf", elapsedSeconds);
        }
        last = end;
    });
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect {
    ...
    // 这种方式可以阻塞帧率和timer回调，因为是卡住runloop，模拟真实开发中超量的运算
